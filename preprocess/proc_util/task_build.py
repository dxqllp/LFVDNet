...
def create_mor_large(adm, events, adm2subj_dict, feature_map, filt_adm_ids=None):
    '''
    mor_adm_icu_id:[(subject_id,hadm_id,None)]
    mor_data:[patient] patient:[feature] feature:[charttime,valuenum]
    mor_label:[hospital_expire_flag]
    '''
    mor_adm_icu_id = []
    mor_data = []
    mor_label = []
    ...
    for _, p_row in tqdm(adm.iterrows(), total=adm.shape[0]):
        ...
        in_time = p.CHARTTIME.min()#入院时间设置为第一次charttime
...
def create_decompensation_large(adm, events, feature_map, icu_dict, los_dict, adm2deathtime_dict, adm2subj_dict, sample_rate=6.0, shortest_length=12.0, future_time_interval=24.0, filt_adm_ids=None):
    '''
    adm_icu_id:[(subject_id,hadm_id,icustay_id)]
    decom_data:icu_sample*feature*(chartime,valuenum)
    decom_label:[lable]
    '''
    eps=1e-6
    adm_icu_id = []
    ...
    for _, p_row in tqdm(adm.iterrows(), total=adm.shape[0]):
        ...
        for icustay_id in icu_dict[adm_id]:
            ...
            if deathtime is None:
                ...
                lived_time = (pd.to_datetime(deathtime) - intime).total_seconds() / 3600.0  # hours
            ...
            for t in sample_times:
                ...
                if mortality == 0:
                    ...
                    cur_mortality = int(lived_time - t < future_time_interval)  # future_time_interval内死亡
...
def create_los_large(adm, events, feature_map, icu_dict, los_dict, adm2subj_dict, sample_rate=12.0, shortest_length=24.0, filt_adm_ids=None):
    '''
    adm_icu_id:[(subject_id,hadm_id,icustay_id)]
    los_data:icu_sample*feature*(chartime,valuenum)
    los_label:[number2cls(los-t)]
    '''
    eps=1e-6
    adm_icu_id = []
    los_data = []
    ...
    for _, p_row in tqdm(adm.iterrows(), total=adm.shape[0]):
        ...
        for icustay_id in icu_dict[adm_id]:
            ...
            for t in sample_times:
                ...
                adm_icu_id.append((adm2subj_dict[adm_id], adm_id, icustay_id))
                los_data.append(patient_icu_sample)
                los_label.append(cur_label)
                
    print("Number of created samples:", len(los_data), len(adm_icu_id), len(los_label))
    print("Number of features:", len(los_data[0]))
    return adm_icu_id, los_data, los_label

def create_wbm_large(adm, events, feature_map, chart_label_dict, icu_dict, los_dict, adm2deathtime_dict, adm2subj_dict, sample_rate=12.0, observ_win=48.0, future_time_interval=1.0, filt_adm_ids=None):
    eps=1e-6
    adm_icu_id = []
    wbm_data = []
    wbm_label = []
