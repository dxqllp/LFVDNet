...
def log_info(opt, phase, epoch, acc, rmse=0.0, start=0.0, value_rmse=0.0, auroc=0.0, auprc=0.0, loss=0.0, f1=0.0, br=0.0, save=False):
    elapsed_time = (time.time() - start) / 60  # 计算经过的时间（分钟）
    print('  -(', phase, ') epoch: {epoch}, RMSE: {rmse:8.5f}, ACC: {acc:8.5f}, '
          'AUROC: {auroc:8.5f}, AUPRC: {auprc:8.5f}, F1: {f1:8.5f}, Brier: {br:8.5f}, '
          'Value_RMSE: {value_rmse:8.5f}, Loss: {loss:8.5f}, Elapse: {elapse:3.3f} min'
          .format(epoch=epoch, rmse=rmse, acc=acc, auroc=auroc, auprc=auprc,
                  f1=f1, br=br, value_rmse=value_rmse, loss=loss, elapse=elapsed_time))

    if save and opt.log is not None:
        with open(opt.log, 'a') as f:
            f.write(phase + ':\t{epoch}, TimeRMSE: {rmse:8.5f}, ACC: {acc:8.5f}, '
                    'AUROC: {auroc:8.5f}, AUPRC: {auprc:8.5f}, F1: {f1:8.5f}, Brier: {br:8.5f}, '
                    'ValueRMSE: {value_rmse:8.5f}, Loss: {loss:8.5f}\n'
                    .format(epoch=epoch, rmse=rmse, acc=acc, auroc=auroc, auprc=auprc,
                            f1=f1, br=br, value_rmse=value_rmse, loss=loss))
...
def evaluate_mc(label, pred, n_class):
    ...
    try:
        f1 = metrics.f1_score(labels_classes, np.argmax(pred, axis=1), average='macro' if n_class > 2 else 'binary')
        br = metrics.brier_score_loss(labels_classes, pred_scores)
    except ValueError:
        f1 = 0
        br = 0

    return acc, auroc, auprc, f1, br
