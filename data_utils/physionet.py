...
class PhysioNet(object):
    urls = ['https://physionet.org/files/challenge-2012/1.0.0/set-a.tar.gz',
    ...
    def __init__(self, root, train=True, download=False,
                 quantization=0.016, n_samples=None, device=torch.device("cpu"), set=4000):
        ...
        if set == 4000:
            self.data = torch.load(os.path.join(self.processed_folder, 'set-a_{}.pt'.format(self.quantization)))
        elif set == 8000:
            train = torch.load(os.path.join(self.processed_folder, 'set-a_{}.pt'.format(self.quantization)))
            valid = torch.load(os.path.join(self.processed_folder, 'set-b_{}.pt'.format(self.quantization)))
            self.data = train+valid
        elif set == 12000:
            train = torch.load(os.path.join(self.processed_folder, 'set-a_{}.pt'.format(self.quantization)))
            valid = torch.load(os.path.join(self.processed_folder, 'set-b_{}.pt'.format(self.quantization)))
            test = torch.load(os.path.join(self.processed_folder, 'set-c_{}.pt'.format(self.quantization)))
    ...
    def download(self):
        ...
        for url in self.urls:
            ...
            outname = 'Outcomes-' + outname.split('-')[1]
            outcomes = torch.load(os.path.join(self.processed_folder, outname + '.pt'))
            print(os.path.join(self.processed_folder, outname + '.pt'))
